...
public class FileTest01 {
	public static void main(String[] args) {
		// File 객체 만들기 연습
		/*
		 * 1. new File (String 파일 또는 경로) => 디렉토리와 디렉토리 사이 또는 디렉토리와 파일명 사이의 구분문자는 '\' 를
		 * 사용하거나 '/'를 사용한다.
		 * 
		 */
		...
		/*
		 * 2. new File (File parent, String child) => parent 디렉토리 안에 있는 child 파일 정보를 갖는
		 * 파일 객체가 생성된다.
		 * 
		 */
		...
		/*
		 * 3. new File (String parent, String child) => parent 디렉토리 안에 있는 child 파일 정보를
		 * 갖는 파일 객체가 생성된다.
		 */
		File file4 = new File("/Users/ChoiSeoYeon/D_Other", "test.txt");
		 ...
		 *
		 * - mkdirs()	=> 중간 부분의 경로가 없으면 중간 부분의 경로를 만들어 준다.
		 */
		...
		File file6 = new File("/Users/ChoiSeoYeon/D_Other/test/java/src");
		if (!file6.exists()) {
			if (file6.mkdirs()) {				
				System.out.println(file6.getName() + "만들기 성공! ^__^");
			} else {
				System.out.println(file6.getName() + "만들기 실패! ^__ㅠ");
			}
		} else {
			System.out.println(file6.getName() + "이미 존재합니다...!");
		}
	}
}
